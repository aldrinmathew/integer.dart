...
/// `0` to `4294967295`
///
/// The `u32` type is contagious. Most operations on `u32` will return `u32`
///  values
class u32 extends unsigned {
  /// 32-bit Unsigned Integer
  ///
  /// `0` to `4294967295`
  u32(int value) : super(value: value.toUnsigned(32));

  /// 32-bit Unsigned Integer
  ///
  /// `0` to `4294967295`
  ///
  /// Create `u32` from another `integer` value
  u32.integer(integer val) : super(value: val.value.toUnsigned(32));
  ...
  /// u32(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// u32.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory u32.fromEnvironment(String name, {int defaultValue = 0}) {
    return u32(
      int.fromEnvironment(
        name,
  ...
  /// Returns `u32`
  @override
  u32 operator *(dynamic other) {
    if (other is integer) {
      return u32(value * other.value);
    } else if (other is int) {
      return u32(value * other);
    } else if (other is double) {
      return u32((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  u32 remainder(dynamic other) {
    if (other is integer) {
      return u32(value.remainder(other.value));
    } else if (other is int) {
      return u32(value.remainder(other));
    } else if (other is double) {
      return u32((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  u32 modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  u32 modInverse(unsigned modulus) => u32(value.modInverse(modulus.value));
