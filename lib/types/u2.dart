...
/// `0` to `3`
///
/// The `u2` type is contagious. Most operations on `u2` will return `u2`
///  values
class u2 extends unsigned {
  /// 2-bit Unsigned Integer
  ///
  /// `0` to `3`
  u2(int value) : super(value: value.toUnsigned(2));

  /// Create `u2` from another `integer` value
  ///
  /// 2-bit Unsigned Integer
  ///
  /// `0` to `3`
  u2.integer(integer val) : super(value: val.value.toUnsigned(2));

  /// Returns the integer value of the given environment declaration [name].
  ///
  /// The result is the same as would be returned by:
  /// ```dart template:expression
  /// u2(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// u2.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  ///
  /// 2-bit Unsigned Integer
  ///
  /// `0` to `3`
  factory u2.fromEnvironment(String name, {int defaultValue = 0}) {
    return u2(
      int.fromEnvironment(
        name,
        defaultValue: defaultValue.toUnsigned(2),
      ),
    );
  }
  ...
  static u2? tryParse(String source) {
  ...
  @override
  u2 modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  u2 modInverse(unsigned modulus) => u2(value.modInverse(modulus.value));
