...
/// `0` to `65535`
///
/// The `u16` type is contagious. Most operations on `u16` will return `u16`
///  values
class u16 extends unsigned {
  /// 16-bit Unsigned Integer
  ///
  /// `0` to `65535`
  u16(int value) : super(value: value.toUnsigned(16));

  /// 16-bit Unsigned Integer
  ///
  /// `0` to `65535`
  ///
  /// Create `u16` from another `integer` value
  u16.integer(integer val) : super(value: val.value.toUnsigned(16));
  ...
  /// u16(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// u16.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory u16.fromEnvironment(String name, {int defaultValue = 0}) {
    return u16(
      int.fromEnvironment(
        name,
        defaultValue: defaultValue.toUnsigned(16),
      ),
    );
  }
  ...
  static u16? tryParse(String source) {
  ...
  u16 operator *(dynamic other) {
    if (other is integer) {
      ...
      return u16((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  u16 remainder(dynamic other) {
    if (other is integer) {
      return u16(value.remainder(other.value));
    } else if (other is int) {
      return u16(value.remainder(other));
    } else if (other is double) {
      return u16((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  u16 modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  u16 modInverse(unsigned modulus) => u16(value.modInverse(modulus.value));
