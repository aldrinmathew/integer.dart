...
/// `-2` to `1`
///
/// The `i2` type is contagious. Most operations on `i2` will return `i2`
/// values
class i2 extends signed {
  /// 2-bit Signed Integer
  ///
  /// `-2` to `1`
  i2(int value) : super(value: value.toSigned(2));

  /// 2-bit Signed Integer
  ///
  /// `-2` to `1`
  ///
  /// Create `i2` from another `integer` value
  i2.integer(integer val) : super(value: val.value.toSigned(2));
  ...
  /// i2(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// i2.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory i2.fromEnvironment(String name, {int defaultValue = 0}) {
    return i2(
      int.fromEnvironment(
        name,
        defaultValue: defaultValue.toSigned(2),
      ),
    );
  }
  ...
  static i2? tryParse(String source) {
  ...
  /// Returns `i2`
  @override
  i2 operator *(dynamic other) {
    if (other is integer) {
      return i2(value * other.value);
    } else if (other is int) {
      return i2(value * other);
    } else if (other is double) {
      return i2((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  i2 remainder(dynamic other) {
    if (other is integer) {
      return i2(value.remainder(other.value));
    } else if (other is int) {
      return i2(value.remainder(other));
    } else if (other is double) {
      return i2((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  i2 modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  i2 modInverse(unsigned modulus) => i2(value.modInverse(modulus.value));
