...
class ux extends unsigned {
  ...
  ux({
    required int bits,
    required int value,
  })  : _bits = bits.clamp(1, 64),
        super(value: value.toSigned(bits.clamp(1, 64)));
  ...
  /// ux(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// ux.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  ///
  /// Signed Integer with custom bit-width
  ///
  ...
  factory ux.fromEnvironment(
    ...
    return ux(
      ...
      value: int.fromEnvironment(
        name,
        defaultValue: defaultValue.toSigned(bits.clamp(1, 64)),
      ),
    );
  }
  ...
  @override
  ux modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  ux modInverse(unsigned modulus) => ofSameBit(value.modInverse(modulus.value));
  ...
  @override
  ix operator -() => ix(bits: bits * 2, value: -value);
}
