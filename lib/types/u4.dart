...
/// `0` to `15`
///
/// The `u4` type is contagious. Most operations on `u4` will return `u4`
///  values
class u4 extends unsigned {
  /// 4-bit Unsigned Integer
  ///
  /// `0` to `15`
  u4(int value) : super(value: value.toUnsigned(4));

  /// 4-bit Unsigned Integer
  ///
  /// `0` to `15`
  ///
  /// Create `u4` from another `integer` value
  u4.integer(integer val) : super(value: val.value.toUnsigned(4));
  ...
  /// u4(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// u4.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory u4.fromEnvironment(String name, {int defaultValue = 0}) {
    return u4(
      int.fromEnvironment(
        name,
        defaultValue: defaultValue.toUnsigned(4),
      ),
    );
  }
  ...
  static u4? tryParse(String source) {
  ...
  u4 operator *(dynamic other) {
    if (other is integer) {
      ...
      return u4((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  u4 remainder(dynamic other) {
    if (other is integer) {
      return u4(value.remainder(other.value));
    } else if (other is int) {
      return u4(value.remainder(other));
    } else if (other is double) {
      return u4((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  u4 modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  u4 modInverse(unsigned modulus) => u4(value.modInverse(modulus.value));
