...
/// `0` and `1`
///
/// The `u1` type is contagious. Most operations on `u1` will return `u1`
///  values
class u1 extends unsigned {
  /// 1-bit Unsigned Integer
  ///
  /// `0` and `1`
  u1(int value) : super(value: value.toUnsigned(1));

  /// Create `u1` from another `integer` value
  ///
  /// 1-bit Unsigned Integer
  ///
  /// `0` and `1`
  u1.integer(integer val) : super(value: val.value.toUnsigned(1));
  ...
  static u1? tryParse(String source) {
  ...
  /// `this`.
  @override
  u1 remainder(dynamic other) {
    if (other is integer) {
      return u1(value.remainder(other.value));
    } else if (other is int) {
      return u1(value.remainder(other));
    } else if (other is double) {
      return u1((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  u1 modPow(unsigned exponent, unsigned modulus) =>
  ...
  @override
  u1 modInverse(unsigned modulus) => u1(value.modInverse(modulus.value));
